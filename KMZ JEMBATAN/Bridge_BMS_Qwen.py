...
Available inputs use its worst-condition/clamping rule.
...
def pdf_has_bridge_id(pages, ident):
    ...
    if not key:
        return False
    pattern = re.compile(r'(?<![\w.-])' + r'\s*'.join(map(re.escape, key)) + r'(?![\w.-])')
...
def apply_pkrms_fields(fields, record, ident):
    ...
    for key in ('Status PKRMS', 'Sumber Koordinat', 'Sumber nilai', 'Sumber'):
        values.pop(key, None)
...
def load_input(path, pkrms=None):
    ...
    for pm_index, pm in enumerate(placemarks, 1):
        ...
        fields = {k: v for k, v in fields.items()
                  if bridge_key(k) not in {'SUMBER KOORDINAT', 'STATUS PKRMS', 'SUMBER NILAI', 'SUMBER'}}
...
def popup(b, title, logo_refs=()):
    ...
    parts = [f'<div style="font-family:Arial,sans-serif;width:560px;color:{NAVY}">',
             ...
             f'<span>Kondisi: {escape(status)}</span></div>',
    ...
    parts.append('<p style="font-size:11px;color:#526676">'
                 'Survey Kondisi Jembatan Kabupaten Tanjung Jabung Barat 2026</p></div>')
    return ''.join(parts)
...
def write_kmz(path, root, payload, kml_name, bridges, title, logos=()):
    ...
    for bridge_index, b in enumerate(bridges, 1):
        ...
        scrubbed_data_names = {'Sumber Koordinat', 'Status PKRMS', 'Sumber nilai', 'Sumber',
                               'bms_nk_status', 'bms_data_authority'}
        for data in list(ex.findall('k:Data', NS)):
            name = data.get('name') or ''
            if (name in values or name == 'qa_length_conflict' or name in scrubbed_data_names
                    or (b.imported and name.startswith(PKRMS_RAW_PREFIX))):
                ex.remove(data)
...
def write_pdf(path, bridges, payload, title, source, logos=()):
        ...
        for i, row in enumerate(rows):
            ...
            if col is not None:
                ...
                cells[i][col] = Paragraph(escape(clean_text(value)), ParagraphStyle('condition', fontName='Helvetica',
                                                                                    ...
                                                                                    textColor=colors.HexColor(condition_ink(value))))
    ...
    def page_decoration(c, doc):
        ...
        if logos:
            x = A4[0] - 36 - 100
            c.setFillColor(colors.white)
            c.roundRect(x, A4[1] - 68, 100, 52, 3, fill=1, stroke=0)
            for i, (data, w, h) in enumerate(logos):
                scale = min(42 / w, 44 / h)
                c.drawImage(ImageReader(io.BytesIO(data)), x + 4 + i * 48 + (42 - w * scale) / 2,
                            A4[1] - 64 + (44 - h * scale) / 2, w * scale, h * scale, mask='auto')

        c.setStrokeColor(colors.HexColor('#D7E1E7'))
        c.line(36, 38, A4[0] - 36, 38)
    ...
    story = [stats, legend, Spacer(1, 12),
    ...
    story += [register, Spacer(1, 15),
              ...
              p('Format ini tidak menyatakan kapasitas struktur atau keamanan operasional. '
                'Kode kerusakan dan rekomendasi penanganan tidak disimpulkan dari foto.')]
    ...
    for i, b in enumerate(bridges, 1):
        ...
        band_text = Paragraph(escape('KONDISI: ' + clean_text(status).upper()),
    ...
    doc = SimpleDocTemplate(str(path), pagesize=A4, leftMargin=36, rightMargin=36,
                            topMargin=105, bottomMargin=52, title=title)
