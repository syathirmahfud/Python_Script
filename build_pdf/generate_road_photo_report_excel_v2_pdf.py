...
FONT_META = ("Helvetica", 6)
FONT_STA = ("Helvetica-Bold", 6)
...
def draw_photos(c, photos, start_y, sections):
    ...
    for row in range(2):
        ...
        for col in range(2):
            ...
            c.drawRightString(img_x + label_w + 2.5 * cm, text_y, ":")

            # STA value
            sta_value = sta.replace("STA_", "").replace("STA ", "")
            c.drawString(img_x + label_w + 3.0 * cm,text_y,sta_value)
            ...
            c.setFont(*FONT_META)
            # Condition
            c.drawString(img_x, text_y - 0.35 * cm, "KONDISI")
            c.drawRightString(img_x + label_w + 2.5 * cm, text_y - 0.35 * cm, ":")
            c.drawString(img_x + label_w + 3.0 * cm,text_y - 0.35 * cm,condition)

            # Pavement
            c.drawString(img_x, text_y - 0.70 * cm, "JENIS PERKERASAN")
            c.drawRightString(img_x + label_w + 2.5 * cm, text_y - 0.70 * cm, ":")
            c.drawString(img_x + label_w + 3.0 * cm,text_y - 0.70 * cm,pavement)

            # Latitude
            c.drawString(img_x, text_y - 0.35 * cm, "Lat")
            c.drawRightString(img_x + label_w + 3 * cm, text_y - 0.35 * cm, ":")
            c.drawString(img_x + label_w + 0.15 * cm,text_y - 0.35 * cm,f"{float(lat):.4f}")

            # Longitude
            c.drawString(img_x, text_y - 0.35 * cm, "Lon")
            c.drawRightString(img_x + label_w + 3 * cm, text_y - 0.70 * cm, ":")
            c.drawString(img_x + label_w + 0.15 * cm,text_y - 0.70 * cm,f"{float(lon):.4f}")
