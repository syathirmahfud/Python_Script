...
def draw_photos(c, photos, start_y, sections):
    ...
    for row in range(2):
        ...
        for col in range(2):
            ...
            img_path = base_photos[idx]
            base, ext = os.path.splitext(img_path)
            inset_path = f"{base}_NR{ext}" if os.path.exists(f"{base}_NR{ext}") else None
            ...
            if inset_path is not None and os.path.exists(inset_path):
                inset_img = ImageReader(inset_path)
                iiw, iih = inset_img.getSize()

                inset_w = draw_w * 0.25
                inset_h = inset_w * iih / iiw
...
def main():
    ...
    logo_left = os.path.join("assets", "sarolangun.png")
    ...
    [
        ...
        if f.lower().endswith(".jpg") and "_NR" not in f
    ],
