...
FONT_TITLE = ("Helvetica-Bold", 12.5)
FONT_SUB = ("Helvetica", 9)
FONT_META = ("Helvetica", 9)
FONT_STA = ("Helvetica-Bold", 5.25)
...
def decimal_to_dms(decimal_deg, is_lat=True):
    ...
    return f"{deg_str}\u00b0  {min_str}'  {sec_str}\"  {ref}"
...
def ask_inputs():
    ...
    return screenshot_dir, save_pdf, excel_file, ruas_number, ruas_name, ruas_length
...
def draw_header(c, logo_left, logo_right, ruas_number, ruas_name, ruas_length):
    ...
    c.drawCentredString(
        PAGE_W / 2,
        y_top - 0.6 * cm - 0.2*cm,
        "DINAS PEKERJAAN UMUM DAN PENATAAN RUANG KAB. TANJUNG JABUNG BARAT"
    )
    ...
    c.drawCentredString(
        PAGE_W / 2,
        y_top - 1.2 * cm - 0.2*cm,
        "Dokumentasi Survey PKRMS Jalan Kab.Tanjung Jabung Barat"
    )

    c.drawCentredString(
        PAGE_W / 2,
        y_top - 1.7 * cm - 0.2*cm,
        "JL. A Jl. Madjid Brangas, Tungkal IV Desa, Kec. Tungkal Ilir, Kabupaten Tanjung Jabung Barat, Jambi 36514"
    )
    ...
    thin_line_y = line_y - 0.1 * cm 
    c.setLineWidth(0.5)
    c.line(MARGIN_L, thin_line_y, PAGE_W - MARGIN_R, thin_line_y)
...
def draw_photos(c, photos, start_y, sections):
    ...
    for row in range(2):
        ...
        for col in range(2):
            ...
            if lat is not None:
                c.drawString(img_x + label_w + 1.75 * cm, text_y - 0.6 * cm, decimal_to_dms(float(lat), is_lat=True))
            else:
                c.drawString(img_x + label_w + 1.75 * cm, text_y - 0.6 * cm, "No GPS data")

            # Longitude
            c.drawString(img_x, text_y - 0.8 * cm, "LONGITUDE")
            c.drawRightString(img_x + label_w + 1.5 * cm, text_y - 0.8 * cm, ":")
            if lon is not None:
                c.drawString(img_x + label_w + 1.75 * cm, text_y - 0.8 * cm, decimal_to_dms(float(lon), is_lat=False))
            else:
                c.drawString(img_x + label_w + 1.75 * cm, text_y - 0.6 * cm, "No GPS data")
            idx += 1
...
# =========================
def main():
    screenshot_dir, save_pdf, excel_file, ruas_number, ruas_name, ruas_length = ask_inputs()

    sections = load_sections(excel_file)

    #logo_left_original = os.path.join("assets", "logo_TanjabBarat.png")
    #logo_left = os.path.join("assets", "logo_TanjabBarat_2048.png")

    #if not os.path.exists(logo_left):
    #    img = Image.open(logo_left_original)
